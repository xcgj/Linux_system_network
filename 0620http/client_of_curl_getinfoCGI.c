...
int main()
{
    ...
    //2 请求URL地址,地址是要cgi-bin的，去找一个cgi的可执行文件
    //  让服务器重定向
    CURLcode code = curl_easy_setopt(curl, CURLOPT_URL, "htttp://127.0.0.1/cgi-bin/a.out");
    if(code != CURLE_OK)
    {
        perror("setopt");
        return 0;
    ...
    curl_easy_setopt(curl, CURLOPT_POST,1);//强制指定用POST方法
