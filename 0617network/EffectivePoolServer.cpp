...
list<Channel*> channellist_free;//空闲的channel对象
...
//对象池---内存碎片的规避---从对象池获取堆空间
Channel * allocChannel(int fd)
{
    Channel * ret = NULL;
    MyLock(mutex);
    if(channellist_free.size() > 0)
    {
        ret = channellist_free.front();
        ret->fd = fd;
        channellist_free.pop_front();
    }
    else
    {
        ret = new Channel(fd);
